...
namespace JWWS\WPPF\WordPress\Repo\Post_Repo\WP_Post_Repo;

use JWWS\WPPF\{
    Common\Security\Security,
    WordPress\Repo\Repo,
    WordPress\Repo\Post_Repo\Post_Repo,
    WordPress\Repo\Post_Type_Repo\WP_Post_Type_Repo\WP_Post_Type_Repo,
...
final class WP_Post_Repo extends Repo implements Post_Repo {
    ...
    public function find_by_id(int $id): \WP_Post {
        ...
        if (! $type_names->contains_value(value: $post->post_type)) {
            throw new \Exception(
                message: "Post with id '{$id}' not found with post type: ss{$type_names->to_string()}.",
            );
        }
